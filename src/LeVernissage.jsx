...
async function fetchShows() {
  ...
  return data.map(s => ({
    ...
    editors_pick: s.editors_pick || false,
...
const T={en:{featured:"Featured",shows:"Shows",map:"Map",reviews:"Reviews",allShows:"All Shows",myPlan:"My Plan",all:"All Current Shows",editorsPicks:"Editor's Picks",closing:"Closing This Week",opening:"Opening This Week",nearby:"Nearby",getDirections:"Get Directions",share:"Share",back:"Back",dates:"Dates",hours:"Hours",area:"Area",noShowsInPlan:"No shows in your plan yet",addFromShows:"Star shows in the Shows tab",locationDenied:"Location access denied.",gettingLocation:"Getting your location…",frameReview:"Frame Review",closingSoon:"Closing",openingSoon:"Opening",onNow:"On Now",away:"away",betweenShows:"Between exhibitions",loading:"Loading…",error:"Could not load shows."},fr:{featured:"En vedette",shows:"Expositions",map:"Carte",reviews:"Critiques",allShows:"Toutes",myPlan:"Mon Plan",all:"Toutes les expositions",editorsPicks:"Sélection",closing:"Ferme cette semaine",opening:"Ouvre cette semaine",nearby:"À proximité",getDirections:"Itinéraire",share:"Partager",back:"Retour",dates:"Dates",hours:"Heures",area:"Quartier",noShowsInPlan:"Aucune exposition dans votre plan",addFromShows:"Ajoutez des expositions depuis Expositions",locationDenied:"Accès refusé.",gettingLocation:"Localisation…",frameReview:"Critique Frame",closingSoon:"Ferme bientôt",openingSoon:"Ouvre bientôt",onNow:"En cours",away:"de vous",betweenShows:"Entre expositions",loading:"Chargement…",error:"Impossible de charger les expositions."}};
...
function statusBadge(s){
  if(isOnNow(s)&&!isOpeningThisWeek(s))return"On Now";
  if(isOpeningThisWeek(s)){const d=Math.ceil((new Date(s.openDate)-TODAY)/86400000);if(d<=0)return"On Now";const days=["Sun","Mon","Tue","Wed","Thu","Fri","Sat"];return`Opening ${days[new Date(s.openDate).getDay()]}`;}
  return null;
}
...
function AdminPage({ onExit }) {
  ...
  const loadShows = async () => {
    ...
    try {
      ...
      const fm = {};
      pending.forEach(s => { fm[s.id] = s.featured || false; });
      setFeaturedMap(fm);

      // Also fetch live (approved) shows for Live Shows section
      const liveRes = await fetch(`${SUPABASE_URL}/rest/v1/shows?status=eq.approved&order=gallery.asc`, {
  ...
  const handleAction = async (id, status) => {
    ...
    if (ok) {
      setPendingShows(prev => prev.filter(s => s.id !== id));
      if (status === "approved") {
        const approved = pendingShows.find(s => s.id === id);
        if (approved) setLiveShows(prev => [approved, ...prev]);
      }
    ...
    return (
      <div style={{ position:"fixed",inset:0,background:INK,zIndex:3000,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:32,maxWidth:430,margin:"0 auto" }}>
        ...
        <div style={{ width:"100%",maxWidth:300 }}>
          <input type="password" value={pwInput} onChange={e=>setPwInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handleLogin()} placeholder="Password" autoFocus
            style={{ width:"100%",padding:"14px 16px",borderRadius:4,fontSize:15,border:`1.5px solid ${pwError?"#E8251A":"#333"}`,background:"#1A1A18",color:WHITE,fontFamily:"'DM Sans',sans-serif",outline:"none",boxSizing:"border-box",transition:"border-color 0.2s",animation:pwError?"shake 0.3s ease":"none" }}/>
    ...
    <div style={{ position:"fixed",inset:0,background:LIGHT,zIndex:3000,display:"flex",flexDirection:"column",maxWidth:430,margin:"0 auto",overflowY:"auto" }}>
      ...
      <div style={{ flex:1,padding:"16px",display:"flex",flexDirection:"column",gap:32 }}>
        ...
        <div>
          ...
          {!loading&&pendingShows.length===0&&(
            <div style={{textAlign:"center",padding:"40px 20px"}}>
              <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontStyle:"italic",color:MID,marginBottom:6}}>All clear</div>
              <div style={{fontSize:13,color:"#AAA"}}>No submissions waiting</div>
            </div>
          )}
                ...
                <div style={{margin:"0 16px 16px",borderRadius:6,border:`2.5px solid ${featuredMap[s.id]?FEATURED_COLOR:BORDER}`,background:featuredMap[s.id]?"#FFF8EC":LIGHT,padding:"14px 16px",transition:"all 0.2s",cursor:"pointer"}} onClick={()=>setFeaturedMap(prev=>({...prev,[s.id]:!prev[s.id]}))}>
                  <div style={{display:"flex",alignItems:"center",justifyContent:"space-between"}}>
                    <div>
                      ...
                      <div style={{fontSize:11,color:featuredMap[s.id]?"#B07800":"#AAA"}}>{featuredMap[s.id]?"Will appear with image card in Featured tab":"OFF — will appear as text card in Shows list"}</div>
                    </div>
                    <div style={{width:52,height:30,borderRadius:15,flexShrink:0,background:featuredMap[s.id]?FEATURED_COLOR:"#CCC",position:"relative",transition:"background 0.2s",boxShadow:featuredMap[s.id]?`0 0 0 3px ${FEATURED_COLOR}44`:"none"}}>
        ...
        <div>
          ...
          {liveShows.length===0&&<div style={{fontSize:13,color:"#AAA",textAlign:"center",padding:"20px 0"}}>No live shows</div>}
          {liveShows.map(s=>(
            <div key={s.id} style={{background:WHITE,borderRadius:6,border:`1px solid ${BORDER}`,padding:"12px 14px",marginBottom:10,display:"flex",alignItems:"center",justifyContent:"space-between",gap:12}}>
              ...
              <button onClick={()=>handleRemove(s.id)} disabled={actionStates[s.id]==="removing"} style={{flexShrink:0,padding:"7px 14px",border:`1px solid #E8251A`,borderRadius:3,background:WHITE,color:"#E8251A",fontSize:11,fontWeight:700,letterSpacing:"0.06em",textTransform:"uppercase",cursor:"pointer",fontFamily:"'DM Sans',sans-serif"}}>
...
function SectionHeader({ title }) {
  return (
    <div style={{ padding:"20px 16px 10px", display:"flex", alignItems:"center", gap:12 }}>
      <div style={{ fontSize:11,letterSpacing:"0.14em",textTransform:"uppercase",color:INK,fontWeight:700 }}>{title}</div>
      <div style={{ flex:1,height:1,background:BORDER }}/>
    </div>
  );
}

// ── Text-only Show Card (non-featured) ───────────────────────────────────────
function TextCard({ s, onClick }) {
  const closing = isClosingThisWeek(s);
  const opening = isOpeningThisWeek(s);
  return (
    <div onClick={onClick} style={{ padding:"14px 16px",borderBottom:`1px solid ${BORDER}`,cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"space-between",gap:12 }}>
      <div style={{ flex:1,minWidth:0 }}>
        <div style={{ fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",color:BLUE,fontWeight:700,marginBottom:3 }}>{s.gallery}</div>
        <div style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontStyle:"italic",fontWeight:600,color:INK,lineHeight:1.2,marginBottom:2 }}>{s.title}</div>
        <div style={{ fontSize:12,color:MID }}>{s.artist}</div>
        <div style={{ fontSize:11,color:MID,marginTop:4 }}>{s.hood}{s.dates?` · ${s.dates}`:""}</div>
      </div>
      <div style={{ display:"flex",flexDirection:"column",alignItems:"flex-end",gap:4,flexShrink:0 }}>
        {closing&&<span style={{fontSize:9,padding:"3px 7px",background:BLUE,color:WHITE,borderRadius:3,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase"}}>Closing</span>}
        {opening&&!closing&&<span style={{fontSize:9,padding:"3px 7px",background:"#22A06B",color:WHITE,borderRadius:3,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase"}}>Opening</span>}
...
function FeaturedCard({ s, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{ cursor:"pointer",borderBottom:`1px solid ${BORDER}` }}>
      <div style={{ width:"100%",height:240,background:s.color,position:"relative",overflow:"hidden" }}>
        {s.image_url&&<img src={s.image_url} alt={s.title} style={{width:"100%",height:"100%",objectFit:"cover",position:"absolute",inset:0}}/>}
        ...
        {badge&&(
          <div style={{ position:"absolute",top:12,left:12,background:badgeColor,color:WHITE,fontSize:10,fontWeight:700,letterSpacing:"0.1em",textTransform:"uppercase",padding:"4px 10px",borderRadius:3 }}>{badge}</div>
        )}
        <div style={{ position:"absolute",bottom:0,left:0,right:0,padding:"14px 16px" }}>
          ...
          <div style={{ fontFamily:"'Cormorant Garamond',serif",fontSize:26,fontStyle:"italic",fontWeight:600,color:WHITE,lineHeight:1.15,marginBottom:2 }}>{s.title}</div>
          <div style={{ fontSize:14,color:"rgba(255,255,255,0.85)" }}>{s.artist}</div>
        </div>
      </div>
      <div style={{ padding:"10px 16px",background:WHITE,display:"flex",justifyContent:"space-between",alignItems:"center" }}>
        <div style={{ fontSize:12,color:MID }}>{s.hood}{s.dates?` · ${s.dates}`:""}</div>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke={MID} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M9 18l6-6-6-6"/></svg>
      </div>
    </div>
  );
}
...
export default function App(){
  ...
  useEffect(()=>{
    fetchShows()
      .then(data=>{setSHOWS(dailyShuffle(data));setLoading(false);})
  ...
  const sourceLabel = detailSource==="featured"?"Featured":detailSource==="shows"?"Shows":detailSource==="map"?"Map":"Reviews";
  ...
  return(
    <div style={{fontFamily:"'DM Sans',sans-serif",background:WHITE,height:"100vh",display:"flex",flexDirection:"column",overflow:"hidden",maxWidth:430,margin:"0 auto",position:"relative",boxShadow:"0 0 60px rgba(0,0,0,0.08)"}}>
      ...
      <div style={{background:WHITE,borderBottom:`1px solid ${BORDER}`,height:52,display:"flex",alignItems:"center",justifyContent:"space-between",padding:"0 20px",flexShrink:0,zIndex:10}}>
        <div onClick={handleHeaderTap} style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontStyle:"italic",fontWeight:600,color:INK,cursor:"default",userSelect:"none"}}>Frame</div>
            ...
            <div style={{flex:1,overflowY:"auto"}}>
              ...
              {!loading&&!loadError&&SHOWS.filter(s=>s.featured&&!s.between).length===0&&(
                <div style={{padding:"60px 20px",textAlign:"center"}}>
                  <div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:24,fontStyle:"italic",color:MID,marginBottom:8}}>No featured shows yet</div>
                  <div style={{fontSize:13,color:"#AAA"}}>Featured exhibitions will appear here</div>
                </div>
              )}
              {!loading&&!loadError&&SHOWS.filter(s=>s.featured&&!s.between).map(s=>(
                <FeaturedCard key={s.id} s={s} onClick={()=>{setDetail(s);setDetailSource("featured");}}/>
              ))}
            </div>
          </div>
        )}
            ...
            <div style={{flex:1,overflowY:"auto"}}>
              ...
              {!loading&&!loadError&&(<>
                ...
                {allCurrent.length>0&&(<>
                  ...
                  {allCurrent.map(s=><TextCard key={s.id} s={s} onClick={()=>{setDetail(s);setDetailSource("shows");}}/>)}
                </>)}
                ...
                {activeHoods.length>0&&(<>
                  ...
                  {activeHoods.map(hood=>{
                    const hoodShows=SHOWS.filter(s=>!s.between&&s.hood===hood&&(isOnNow(s)||isOpeningThisWeek(s)));
                    return hoodShows.length>0?(
                      <div key={hood}>
                        <div style={{padding:"12px 16px 4px",fontSize:10,letterSpacing:"0.14em",textTransform:"uppercase",color:MID,fontWeight:700}}>{hood}</div>
                        {hoodShows.map(s=><TextCard key={s.id} s={s} onClick={()=>{setDetail(s);setDetailSource("shows");}}/>)}
                      </div>
                    ):null;
                  })}
                </>)}
                ...
                {editorsPicks.length>0&&(<>
                  ...
                  {editorsPicks.map(s=><TextCard key={s.id} s={s} onClick={()=>{setDetail(s);setDetailSource("shows");}}/>)}
                </>)}

                {/* Opening This Week */}
                {openingThisWeek.length>0&&(<>
                  <SectionHeader title={t.opening}/>
                  {openingThisWeek.map(s=><TextCard key={s.id} s={s} onClick={()=>{setDetail(s);setDetailSource("shows");}}/>)}
                </>)}

                {/* Closing This Week */}
                {closingThisWeek.length>0&&(<>
                  <SectionHeader title={t.closing}/>
                  {closingThisWeek.map(s=><TextCard key={s.id} s={s} onClick={()=>{setDetail(s);setDetailSource("shows");}}/>)}
                </>)}

                {/* Bottom CTA section */}
                <div style={{padding:"32px 20px 40px",borderTop:`1px solid ${BORDER}`,marginTop:8}}>
                  <div style={{marginBottom:24}}>
                    <div style={{fontSize:13,fontWeight:600,color:INK,marginBottom:4}}>List your show for free</div>
                    <div style={{fontSize:12,color:MID,marginBottom:12}}>Submit your exhibition to appear in the Shows list.</div>
                    <a href="https://tally.so/r/D4Je7b" target="_blank" rel="noopener noreferrer" style={{display:"inline-block",padding:"10px 20px",background:INK,color:WHITE,borderRadius:4,fontSize:11,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",textDecoration:"none"}}>Submit Exhibition</a>
                  </div>
                  <div style={{marginBottom:24}}>
                    <div style={{fontSize:13,fontWeight:600,color:INK,marginBottom:4}}>Stand out with an image card</div>
                    <div style={{fontSize:12,color:MID,marginBottom:12}}>Feature your exhibition at the top of the app with a full image card.</div>
                    <a href="mailto:hello@useframe.ca" style={{display:"inline-block",padding:"10px 20px",border:`1.5px solid ${INK}`,color:INK,borderRadius:4,fontSize:11,fontWeight:700,letterSpacing:"0.08em",textTransform:"uppercase",textDecoration:"none"}}>Get in Touch</a>
                  </div>
                  <div style={{display:"flex",flexDirection:"column",gap:10}}>
                    <a href="mailto:hello@useframe.ca" style={{fontSize:12,color:MID,textDecoration:"none"}}>✉ Questions → hello@useframe.ca</a>
                    <a href="https://instagram.com/useframe.ca" target="_blank" rel="noopener noreferrer" style={{fontSize:12,color:MID,textDecoration:"none"}}>📷 Follow us on Instagram</a>
                    <button onClick={()=>{if(navigator.share){navigator.share({title:"Frame",text:"Discover Montreal art exhibitions",url:"https://useframe.ca"});}}} style={{background:"none",border:"none",padding:0,fontSize:12,color:MID,cursor:"pointer",textAlign:"left",fontFamily:"'DM Sans',sans-serif"}}>↗ Share Frame with a friend</button>
                  </div>
                </div>
      ...
      {/* ── Bottom Tab Bar ── */}
      <div style={{background:WHITE,borderTop:`1px solid ${BORDER}`,display:"flex",flexShrink:0,paddingBottom:"env(safe-area-inset-bottom)",zIndex:10}}>
        {tabs.map(({key,label,icon})=>{
          const active=tab===key;
          return(
            <button key={key} onClick={()=>setTab(key)} style={{flex:1,padding:"10px 0 8px",display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:3,background:"transparent",border:"none",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",position:"relative"}}>
      ...
      {detail&&(
        <div style={{position:"fixed",inset:0,background:WHITE,zIndex:2000,overflowY:"auto",animation:"slideUp 0.32s cubic-bezier(0.16,1,0.3,1)",maxWidth:430,margin:"0 auto"}}>
          <div style={{position:"sticky",top:0,background:WHITE,borderBottom:`1px solid ${BORDER}`,height:52,display:"flex",alignItems:"center",padding:"0 4px 0 0",zIndex:10}}>
            <button onClick={()=>setDetail(null)} style={{height:"100%",padding:"0 20px",display:"flex",alignItems:"center",gap:6,background:"none",border:"none",cursor:"pointer",fontFamily:"'DM Sans',sans-serif",minWidth:44}}>
          ...
          <div style={{width:"100%",height:detail.between?140:260,background:detail.between?LIGHT:detail.color,display:"flex",alignItems:"center",justifyContent:"center",position:"relative",overflow:"hidden"}}>
            ...
            {detail.between&&<div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:20,fontStyle:"italic",color:MID}}>Between exhibitions</div>}
          </div>
          <div style={{padding:"24px 20px"}}>
            ...
            {detail.reviewed&&detail.quote&&(<div style={{background:"#EEF2FD",borderLeft:`3px solid ${BLUE}`,padding:"18px 16px",marginBottom:24,borderRadius:"0 4px 4px 0"}}><div style={{fontSize:10,letterSpacing:"0.12em",textTransform:"uppercase",color:BLUE,fontWeight:700,marginBottom:10}}>{t.frameReview}</div><div style={{fontFamily:"'Cormorant Garamond',serif",fontSize:18,fontStyle:"italic",lineHeight:1.8,marginBottom:10,color:INK}}>{detail.quote}</div><div style={{fontSize:12,color:MID}}>{detail.by}</div></div>)}
